...
public class GuardModelAdapter<T extends Searchable>
        extends RecyclerView.Adapter<GuardModelAdapter.ViewHolder> {
    ...
    public GuardModelAdapter(Context context, @LayoutRes int layout, List<T> items) {
        this(context,layout,null, items);
    }

    public GuardModelAdapter(Context context, AdapterViewBinder<T> viewBinder,
                             @LayoutRes int layout, List<T> items) {
        this(context,layout,viewBinder, items);
    }

    public GuardModelAdapter(Context context, @LayoutRes int layout,
    ...
    public GuardModelAdapter<T> setViewBinder(AdapterViewBinder<T> viewBinder) {
        this.mViewBinder = viewBinder;
        return this;
    }
    ...
    @Override
    public void onBindViewHolder(GuardModelAdapter.ViewHolder holder, int position) {
        initializeViews(getItem(position), holder, position);
    }

    private void initializeViews(final T object, final GuardModelAdapter.ViewHolder holder,
                                 final int position) {
        if(mViewBinder != null)
        ...
        text.setText(((Guard) object).getFullname());
        //subText.setText(((Guard) object).dni);
        subText.setVisibility(View.GONE);
        if (((Guard) object).photo == null) {
    ...
    public GuardModelAdapter setSearchTag(String searchTag) {
    ...
    public GuardModelAdapter setHighlightPartsInCommon(boolean highlightPartsInCommon) {
    ...
    public GuardModelAdapter setHighlightColor(String highlightColor) {
        mHighlightColor = highlightColor;
        return this;
    }

    public GuardModelAdapter setSearchDialog(BaseSearchDialogCompat searchDialog) {
