...
public class UserModelAdapter<T extends Searchable>
        extends RecyclerView.Adapter<UserModelAdapter.ViewHolder> {
    ...
    public UserModelAdapter(Context context, @LayoutRes int layout, List<T> items) {
        this(context,layout,null, items);
    }

    public UserModelAdapter(Context context, AdapterViewBinder<T> viewBinder,
                            @LayoutRes int layout, List<T> items) {
        this(context,layout,viewBinder, items);
    }

    public UserModelAdapter(Context context, @LayoutRes int layout,
    ...
    public UserModelAdapter<T> setViewBinder(AdapterViewBinder<T> viewBinder) {
        this.mViewBinder = viewBinder;
        return this;
    }
    ...
    @Override
    public void onBindViewHolder(UserModelAdapter.ViewHolder holder, int position) {
        initializeViews(getItem(position), holder, position);
    }

    private void initializeViews(final T object, final UserModelAdapter.ViewHolder holder,
                                 final int position) {
        if(mViewBinder != null)
        ...
        if (object instanceof Admin) {
            ...
            subText.setText(admin.getFullname());
            ...
            subText.setText(guard.getFullname());
            subText.setVisibility(View.VISIBLE);
            if (guard.photo == null) {
                subText.setVisibility(View.GONE);
                image.setImageDrawable(mContext.getDrawable(R.drawable.policeman));
    ...
    public UserModelAdapter setSearchTag(String searchTag) {
    ...
    public UserModelAdapter setHighlightPartsInCommon(boolean highlightPartsInCommon) {
    ...
    public UserModelAdapter setHighlightColor(String highlightColor) {
        mHighlightColor = highlightColor;
        return this;
    }

    public UserModelAdapter setSearchDialog(BaseSearchDialogCompat searchDialog) {
